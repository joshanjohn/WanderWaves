...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $tenant_agreement = validate_input($_POST['agreement']);

    if (
        !(
            $name && $address && $eircode && $category && $price && $start_date &&
            $end_date && $description && $num_beds && $size && $tenant_agreement && $property_id
        )
    )
    ...
    if ($error) {
        // SQL statement with prepared statement // ===> Check this
        $sql_query = $db_connection->prepare(
            "UPDATE property SET name=?, address=?, eircode=?, category=?, price=?, 
                start_date=?, end_date=?, description=?, num_beds=?, size=?, agreement=? 
                WHERE property_id=?"
        );
        if ($sql_query) {
            // Bind parameters to the prepared statement // ===> Check this
    ...
    // Close database connection
    $db_connection->close();
    header("Location:../pages/property/property_edit.php"); 
}
?>